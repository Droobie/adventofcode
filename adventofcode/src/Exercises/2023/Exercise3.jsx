...
const findDigitIndicesAndLengths = (inputString, regex) => {
  const digitRegex = regex;
  let match;
  const result = [];

  // yes a while... dont kill me
  while ((match = digitRegex.exec(inputString)) !== null) {
      result.push({ index: match.index, length: match[0].length, number: Number(match[0]), rowLength: inputString.length });
  }

  return result;
}

const checkRowPartOne = ({row, start, end}) => {
  let hasMatch = false;
...
const partOne = () => {
  ...
  // empty rows at first and last position because iam to bored to add checks for it
  const numberRows = [[]];
  const symbolRows = [[]];

  data.split(/\r?\n/g).map((row, index) => {
    numberRows.push(findDigitIndicesAndLengths(row, /\d+/g));
    symbolRows.push(findDigitIndicesAndLengths(row, /[^\d.]+/g));
  });
  ...
  const matchingNumbers = numberRows.map((row, index) => {
    return row.map((number) => {
      if (!isNaN(number.number)) {
        const hasMatch = checkRowPartOne({
          row: symbolRows[index - 1],
          start: number.index !== 0 ? number.index - 1 : number.index,
          end: number.index + number.length < number.rowLength ? number.index + number.length : number.index + number.length
        }) || checkRowPartOne({
          row: symbolRows[index + 1],
          start: number.index !== 0 ? number.index - 1 : number.index,
          end: number.index + number.length < number.rowLength ? number.index + number.length : number.index + number.length
        }) || checkRowPartOne({
          row: symbolRows[index],
          start: number.index !== 0 ? number.index - 1 : 0,
          end: number.index + number.length < number.rowLength ? number.index + number.length : number.index + number.length
        })

        console.log(number.number, hasMatch);

        totalCount = hasMatch ? totalCount + number.number : totalCount;
      }
    })
  })
