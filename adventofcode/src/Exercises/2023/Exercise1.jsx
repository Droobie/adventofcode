...
const partTwo = () => {
  const output = data.split(/\r?\n/g).map((row) => {
    const regex = new RegExp(Object.keys(lettersToDigit).join("|"),"gi");

    const convertedLetters = row.replace(regex, function(matched){
      return lettersToDigit[matched];
    });

    return [...convertedLetters.replace(/[^0-9]+/g, '')];
  }).map((numberArr) => {
    return Number(`${numberArr[0]}${numberArr[numberArr.length - 1]}`)
  }).reduce((acc, value) => {
    if (!isNaN(value)) {
