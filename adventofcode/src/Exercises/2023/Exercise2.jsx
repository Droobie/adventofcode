const data = `Game 1: 1 red, 10 blue, 5 green; 11 blue, 6 green; 6 green; 1 green, 1 red, 12 blue; 3 blue; 3 blue, 4 green, 1 red
...
const partOne = () => {
  ...
  let calculatedGameIds = 0;

  data.split(/\r?\n/g).map((row) => {
    const gameId = row.split(':')[0].replace(/[^0-9]+/g, '');

    const hasInvalidValues = row.split(':')[1].split(';').map((grab) => {
      return grab.split(',').map((color) => {
        const splitColor = color.split(' ');
        return allowedLoad[splitColor[2]] < Number(splitColor[1]);
      }).filter(Boolean);
    }).filter((row) => {
      return row.length ? 'nope' : null
    })

    calculatedGameIds = !hasInvalidValues.length ? calculatedGameIds + Number(gameId) : calculatedGameIds;
  });

  return calculatedGameIds;
};
...
const partTwo = () => {
  ...
  data.split(/\r?\n/g).map((row) => {
    ...
    row.split(':')[1].split(';').map((grab) => {
      return grab.split(',').map((color) => {
        const splitColor = color.split(' ');
        if (minimumData[splitColor[2]] < Number(splitColor[1])) {
          minimumData[splitColor[2]] = Number(splitColor[1]);
        }
      });
    })

    totalCount += Object.values(minimumData).reduce((a, b) => a * b, 1);
  });
  return totalCount;
};
...
export const Exercise2 = () => {
  return (
    <>
      <p>answer part 1: {partOne()}</p>
      <p>answer part 2: {partTwo()}</p>
    </>
  );
};
