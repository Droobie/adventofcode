...
const splitItem = (item) => {
  const splitOnColon = item.split(":");
  const splitOnSpace = splitOnColon[0].split(" ");
  const splitOnDash = splitOnSpace[0].split("-");

  return {
    first: Number(splitOnDash[0]),
    second: Number(splitOnDash[1]),
    char: splitOnSpace[1],
    pass: splitOnColon[1],
  };
};

const validPass1 = ({ first, second, char, pass }) => {
  const amount = (pass.match(new RegExp(char, "g")) || []).length;
  return amount >= first && amount <= second;
};

const validPass2 = ({ first, second, char, pass }) => {
  const chars = [pass.charAt(first), pass.charAt(second)];
  return chars.includes(char) && chars[0] !== chars[1];
};

const partOne = () => {
  return list.filter((item) => {
    return validPass1(splitItem(item)) ? item : null;
  }).length;
};

const partTwo = () => {
  return list.filter((item) => {
    return validPass2(splitItem(item)) ? item : null;
  }).length;
};
...
export const Exercise2 = () => {
  return (
    <>
      <p>answer part 1: {partOne(list)}</p>
      <p>answer part 2: {partTwo(list)}</p>
    </>
  );
};
