...
// get an array of all documents by splitting on 2 linebreaks
...
const byrValidator = (part) => {
  return (
    part.match(new RegExp("^[0-9]{4}$", "g")) &&
    Number(part) >= 1920 &&
    Number(part) <= 2002
  );
};

const iyrValidator = (part) => {
  return (
    part.match(new RegExp("^[0-9]{4}$", "g")) &&
    Number(part) >= 2010 &&
    Number(part) <= 2020
...
const hgtValidator = (part) => {
  return;
};

const hclValidator = (part) => {
  return part.match(new RegExp("^#([0-9a-fA-F]{6})$", "g"));
};

const eclValidator = (part) => {
  return ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].includes(part);
};

const pidValidator = (part) => {
  return part.match(new RegExp("^[0-9]{9}$", "g"));
};
...
const verifyDocument = (document) => {
  ...
  return (
    document.reduce((result, part) => {
      console.log(part.split(":")[0]);
      result = validationMapper[part.split(":")[0]](part.split(":")[1])
        ? result + 1
        : result;
...
const isValidDoc = (file, verifyData) => {
  const REQUIRED_PARTS = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];

  const splittedFile = file.split(" ");

  const hasAllNeededItems =
    splittedFile.reduce((result, part) => {
      result = REQUIRED_PARTS.includes(part.split(":")[0])
        ? result + 1
        : result;

      return result;
    }, 0) === REQUIRED_PARTS.length;

  return verifyData
    ? hasAllNeededItems && verifyDocument(splittedFile)
    : hasAllNeededItems;
};

const partOne = (file) => {
  const formattedFile = formatFile(file);
  return formattedFile.reduce((result, file) => {
    result = isValidDoc(file, false) ? result + 1 : result;
    return result;
  }, 0);
};
...
const partTwo = (file) => {
  const formattedFile = formatFile(file);
  return formattedFile.reduce((result, file) => {
    result = isValidDoc(file, true) ? result + 1 : result;
    return result;
  }, 0);
};
...
export const Exercise4 = () => {
  return (
    <>
      <p>answer part 1: {partOne(file)}</p>
      <p>answer part 2: {partTwo(file)}</p>
    </>
  );
};
